	...
	for seed = 1:1
		...
		for (nn, na) in product(nns, nas)
			...
			classifier_parameters = (λ=lambda, batchsize=batchsize, nepochs=200, val_ratio=0.2, 
				n_candidates=100, use_reconstruction=false)    
...
outpath = datadir("baseline_comparison/had_no_rec")
mkpath(outpath)
...
for mf in mfs
	for ilabel in 1:8
		global labels = label_vec[ilabel]
		global desc = label_desc[ilabel]
